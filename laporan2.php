        ...
        <table width="1000" border="1" align="center">
          ...
          <?php
            ...
            if ($bilik == "-" && $bulan == "-" && $tahun == "-") {
              ...
              $data1 = mysqli_query($samb, "SELECT * FROM tempahan WHERE idbilik='$bilik' AND (MONTH(tarikh_masuk)='$bulan' 
              OR MONTH(tarikh_keluar)='$bulan') ORDER BY idbilik,tarikh_masuk");
            } elseif ($bilik != "-" && $bulan != "-" && $tahun != "-") {
              $data1 = mysqli_query($samb, "SELECT * FROM tempahan WHERE idbilik='$bilik' AND ((MONTH(tarikh_masuk)='$bulan' 
              AND YEAR(tarikh_masuk)='$tahun') OR (MONTH(tarikh_keluar)='$bulan' AND YEAR(tarikh_keluar)='$tahun')) 
              ORDER BY idbilik,tarikh_masuk");
            } elseif ($bilik == "-" && $bulan != "-" && $tahun == "-") {
              $data1 = mysqli_query($samb, "SELECT * FROM tempahan WHERE MONTH(tarikh_masuk)='$bulan' OR MONTH(tarikh_keluar)='$bulan' 
              ORDER BY idbilik,tarikh_masuk");
            } elseif ($bilik == "-" && $bulan !== "-" && $tahun != "-") {
              $data1 = mysqli_query($samb, "SELECT * FROM tempahan WHERE YEAR(tarikh_masuk)='$tahun' OR YEAR(tarikh_keluar)='$tahun' 
              ORDER BY idbilik,tarikh_masuk");
            } elseif ($bilik != "-" && $bulan == "-" && $tahun != "-") {
              $data1 = mysqli_query($samb, "SELECT * FROM tempahan WHERE idbilik='$bilik' AND YEAR(tarikh_masuk)='$tahun' 
              OR YEAR(tarikh_keluar)='$tahun') ORDER BY idbilik,tarikh_masuk");
            }
            $jumBesar = 0;
