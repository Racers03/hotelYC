...
<?php echo date("d/m/Y"); ?></div>
 ...
 <?php
   ...
   {
       $data1=mysqli_query($samb,"SELECT * FROM tempahan
       WHERE idbilik='$bilik' AND (MONTH(tarikh_masuk)='$bulan'
       OR MONTH(tarikh_keluar)='$bulan')
       ORDER BY idbilik,tarikh_masuk");
   }
    elseif ($bilik!="-"&&$bulan!="-"&&$tahun!="-")
   {
       $data1=mysqli_query($samb,"SELECT * FROM tempahan
       WHERE idbilik='$bilik' AND ((MONTH(tarikh_masuk)='$bulan'
       AND YEAR(tarikh_masuk)='$tahun')
       OR (MONTH(tarikh_keluar)='$bulan' AND YEAR(tarikh_keluar)='$tahun'))
       ORDER BY idbilik,tarikh_masuk");
   }
        elseif ($bilik=="-"&&$bulan!="-"&&$tahun=="-")
   {
   ...
   {
        $data1=mysqli_query($samb,"SELECT * FROM tempahan
        WHERE idbilik='$bilik' AND YEAR(tarikh_masuk)='$tahun'
        OR YEAR(tarikh_keluar)='$tahun')
        ORDER BY idbilik,tarikh_masuk");
   }
